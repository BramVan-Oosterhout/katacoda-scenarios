...
*'X' allows 'Y'* is true if web/topic 'X' exists and allows access mode 'Y' for the current user... 
...
Throughout wiki applications you will encounter and use the BASEWEB and BASETOPIC macros.Wiki applications use DBCALL or INCLUDE a lot. To implement stricter security, Foswiki has implemented the INCLUDE  so that it expands the INCLUDEd topic content as if you were opening the included topic in the web where the topic resides. For instance: `DBCALL{ "Applications.RenderSimpleTopicCreator" }` is expanded within the security context of the Applications web.  BASEWEB and BASETOPIC serve that purpose.
